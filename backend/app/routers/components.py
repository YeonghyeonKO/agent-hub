...
async def get_file_content(
    ...
    raise HTTPException(status_code=404, detail="File not found — no disk or DB content")


@router.delete("/{component_id}")
async def delete_own_component(
    component_id: uuid.UUID,
    db: Annotated[AsyncSession, Depends(get_db)],
    user: Annotated[User, Depends(get_current_user)],
):
    ...
    if component.status not in ("pending", "draft", "rejected") and user.role != "admin":
        raise HTTPException(status_code=400, detail="Only pending/draft/rejected items can be deleted by the author")
